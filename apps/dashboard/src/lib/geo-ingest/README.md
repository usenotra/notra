...
A valid token is not enough to write an event. After the URL parses, ingest loads the project's brand website plus `geo_settings.domains` and drops the request (same silent 202 as an untracked visitor) when the host is not on that list. Org-scoped tokens union every project in the organization. An empty allowlist drops everything. A lookup outage fails open so a database blip does not discard real traffic. The token stays an HMAC of org/project/generation — binding hosts in the signature would rotate on every settings edit.
