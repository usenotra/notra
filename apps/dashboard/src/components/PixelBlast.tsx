  ...
  useEffect(() => {
    ...
    if (container) {
      ...
      try {
        ...
        // Decorative only: missing WebGL must not take down the auth forms.
      }
      if (renderer) {
        ...
        const uniforms = {
          ...
          uShapeType: { value: SHAPE_MAP[visual.variant] ?? 0 },
          uPixelSize: { value: visual.pixelSize * renderer.getPixelRatio() },
          uScale: { value: visual.patternScale },
          uDensity: { value: visual.patternDensity },
          uPixelJitter: { value: visual.pixelSizeJitter },
          uEnableRipples: { value: visual.enableRipples ? 1 : 0 },
          uRippleSpeed: { value: visual.rippleSpeed },
          uRippleThickness: { value: visual.rippleThickness },
          uRippleIntensity: { value: visual.rippleIntensityScale },
          uEdgeFade: { value: visual.edgeFade },
        };
        ...
        const setSize = () => {
          ...
          uniforms.uPixelSize.value =
            pixelSizeRef.current * renderer.getPixelRatio();
        };
        setSize();
        ...
        if (liquid) {
          ...
          touch.radiusScale = visual.liquidRadius;
          composer = new EffectComposer(renderer);
          const renderPass = new RenderPass(scene, camera);
          liquidEffect = createLiquidEffect(touch.texture, {
            strength: visual.liquidStrength,
            freq: visual.liquidWobbleSpeed,
          });
        ...
        pointerCanvas = renderer.domElement;
    ...
    return () => {
      ...
      if (pointerCanvas && onPointerMove) {
