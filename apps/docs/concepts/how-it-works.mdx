...
<Steps>
  ...
  <Step title="Scan">
    On every scan Notra sends each prompt to engines that search the live web: ChatGPT, Claude, Gemini and Perplexity, plus Google AI Overview and coding agents that research live. Notra keeps the full answer and the search queries and cited sources. A separate judge step checks every answer for mentions of your brand, your aliases and each tracked competitor.
  </Step>
...
Onboarding tracks ChatGPT, Claude, Gemini and Perplexity. GEO Settings exposes a larger model catalog; only models with a live web-search route are scanned. Scans in additional languages run a translated copy of each prompt.
