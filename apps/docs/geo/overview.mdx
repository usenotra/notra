...
The **Brand Sentiment** dashboard tab uses the same two-module layout as Visibility:
a score and period-comparison trend on the left, with agent-extracted themes and
the positive, neutral and negative distribution on the right. The modules stack on mobile.
The distribution includes an exact-proportion stacked bar and a textual count/percentage
legend. Sampling and denominator details are available in compact disclosures.
Expand **Provider scores** for the compact provider breakdown. Each row shows the family's logo,
...
These rows are read-only summaries for the selected project and date range.
...
selected inclusive window. The two observed lines align by day within their respective
periods, and tooltips identify both dates. Toggle either period independently.
...
Select **Analyze answers** to extract themes from saved positive and negative answers.
This is a separate structured model call through the organization's existing model router,
using `openai/gpt-4.1-mini`. It has no browsing or other tools. Each attempted call uses
...
The UI reports sampled and eligible answer counts. The model may return up to six themes,
...
Expand a theme to see its saved quotes, engines, UTC dates and prompts. **Recurring** means
...
analysis version, brand name and historical-input fingerprint. The fingerprint includes
...
That lookup is marked stale and displays **Analysis missing or out of date** with an explicit instruction
to select **Analyze answers**. A completed scan does not itself generate themes.
Generation is explicit, rather than automatically starting another paid call after a scan.

The UI handles ready, pending, stale, failed and unavailable states. A 180-second Redis
lease prevents concurrent extraction for the same input. The worker checks ownership after
...
unavailable. No schema migration is required.
