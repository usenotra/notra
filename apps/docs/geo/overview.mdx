...
The **Brand Sentiment** dashboard tab uses two native dualtone cards:
a compact score and period-comparison trend on the left, with the positive, neutral
and negative distribution on the right. A standalone themes table sits below the cards.
The cards stack on mobile. Loading uses layout-preserving skeletons; empty states
distinguish no saved answers, no rated mentions and no supported themes.
...
periods, and tooltips identify both dates. Toggle either period independently in **Periods**.
...
A completed scan does not itself generate themes.
