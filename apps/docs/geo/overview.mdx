...
The **Brand Sentiment** dashboard tab uses two native dualtone cards side by side:
a wide current-period chart on the left and a compact **Current sentiment score** card on the right.
...
Below, the same table component used elsewhere in GEO offers **By theme** and
**Supporting answers** views. The first lists themes, sentiment, models and distinct
supporting-answer counts. The second lists the original prompts for the cited answers,
deduplicated by saved check. It shows the analyzed evidence sample, not all responses
in the selected period. Select a row to open the original quotes, model and timestamp
in a detail sheet. Multiple quoted passages from the same answer remain available.
On mobile, sentiment appears beneath the theme or prompt; model details remain in
the sheet. Sampling details sit below the table.
