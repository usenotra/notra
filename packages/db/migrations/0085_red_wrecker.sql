...
CREATE INDEX "geoMentionChecks_project_captured_cover_idx" ON "geo_mention_checks" USING btree ("project_id","captured_at","organization_id","language","engine","prompt_id","mentioned","position","sentiment","sequence_id");--> statement-breakpoint
CREATE INDEX "geoMentionChecks_project_prompt_engine_captured_idx" ON "geo_mention_checks" USING btree ("project_id","prompt_id","engine","captured_at" DESC NULLS LAST);--> statement-breakpoint
...
LOCK TABLE "members" IN SHARE ROW EXCLUSIVE MODE;--> statement-breakpoint
