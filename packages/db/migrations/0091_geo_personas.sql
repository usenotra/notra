...
CREATE INDEX "geoMentionChecks_personaId_capturedAt_idx" ON "geo_mention_checks" USING btree ("persona_id","captured_at");--> statement-breakpoint
ALTER TABLE "geo_mention_checks" ADD CONSTRAINT "geoMentionChecks_personaSnapshot_check" CHECK ("geo_mention_checks"."persona_id" IS NULL OR "geo_mention_checks"."persona_snapshot" IS NOT NULL);